...
class Tetris extends Component {
  ...
  pieceCells = (x, y, piece, board, dir) => {
    const cells = []
    const orgX = x
    let count = 0
    for (let bit = 0x8000; bit > 0; bit = bit >> 1) {
      ...
      {
        if (this.checkPlacement(x, y, board) === false)
          return false
        cells.push([x, y])
      }
      x++;
  ...
  update = (xInc, yInc) => (prevState) => {
    const {x, y, board, piece, oldPieceCells, dir, score} = prevState 
    this.placeCells(board, oldPieceCells, 0)
    const cells = this.pieceCells(x + xInc, y + yInc, piece, board, dir)
    if (cells === false && xInc) {
      this.placeCells(board, oldPieceCells, 1)
    }
    else if (cells === false && yInc) {
      this.placeCells(board, oldPieceCells, 1)
      const {pointsScored, newBoard} = this.removeFilledLines(board)
      return ({
        board: newBoard,
        piece: this.newPiece(),
        x: width / 2,
  ...
  switchPieceDir = () => (prevState) => {
    const {x, y, piece, board, dir, oldPieceCells} = prevState
    const newDir = (dir + 1) % 4
    this.placeCells(board, oldPieceCells, 0)
    var r = this.pieceCells(x, y, piece, board, newDir)
    this.placeCells(board, oldPieceCells, 1)
